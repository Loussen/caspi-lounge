...
class ControllerCatalogProduct extends Controller {
        ...
        if ( $this->request->server['REQUEST_METHOD'] == 'POST' )
        {
            ...
            foreach ($this->request->post['product_seo_url'] as $store_id => $language)
            {
                foreach ($language as $language_id => $keyword)
                {
                    ...
                    $query_home_url = $this->model_design_seo_url->getHomeUrl($store_id, $language_id);

                    if ( !empty($query_home_url['keyword']) )
                    {
                        $keyword = str_replace($query_home_url['keyword'] ."/", "", $keyword);
                    }

                    if ( empty($keyword) && !empty($this->request->post['product_description'][$this->config->get('config_language_id')]['name']) ) {
                        $keyword = autocrateseourls( trim($this->request->post['product_description'][$this->config->get('config_language_id')]['name']) );
                    } else {
                        $keyword = autocrateseourls( trim($keyword) );
                    }

                    if ( !empty($query_home_url['keyword']) )
                    {
                        $keyword = $query_home_url['keyword'] ."/". $keyword;
                    }
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				'name'       => $result['name'],
				'model'      => $result['model'],
				'sku'        => (isset($result['sku']) && !empty($result['sku']) ? $result['sku'] : "Null"),
	...
	protected function getForm() {
		...
		if (isset($this->request->post['manufacturer'])) {
			...
			$manufacturer_info = $this->model_catalog_manufacturer->getManufacturerDescriptions($product_info['manufacturer_id']);

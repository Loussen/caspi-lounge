...
//		echo "<hr>";
        ...
        foreach ($results as $result) {
            ...
            $product_say = $this->model_catalog_product->getTotalProducts( array( 'filter_manufacturer_id' => $result['manufacturer_id']) );


            $data['zu_manufacturers'][] = array(
                'name' => $result['name'],
                'href' => $this->url->link('product/manufacturer/info', 'manufacturer_id=' . $result['manufacturer_id']),
                'key' => $key,
                'product_count' => $product_say,
            );
...
//			echo "</pre>\n";
    ...
    public function info() {
        ...
        if ($manufacturer_info) {
            $this->document->setTitle( ($manufacturer_info['meta_title'] ? $manufacturer_info['meta_title'] : $manufacturer_info['name'] ." - ". $this->config->get('config_name')) );
            $this->document->setDescription( $manufacturer_info['meta_description'] ? $manufacturer_info['meta_description'] : mb_substr(strip_tags(htmlspecialchars_decode($manufacturer_info['description'])),0,155, "utf-8") );
            ...
            foreach ($results as $result) {
                ...
                $data['products'][] = array(
                    ...
                    'href'        => $this->url->link('product/product', (isset($path) ? 'path=' . $path . '&' : '') .'product_id=' . $result['product_id'] . $url)
                );
            }
